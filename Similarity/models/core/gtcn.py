...
class GTCN(torch.nn.Module):
    def __init__(self, encoder, aggr, n_node_feat_dim, n_edge_feat_dim, layer_groups, n_message_net_layers, skip_mode,
        ...
        # GTCN
        self.conv1 = GATConv(n_node_feat_dim, n_node_feat_dim // 4, add_self_loops=True, heads=4, concat=True)
    ...
    def forward(self, x, edge_index, edge_feat, graph_idx, batch_size: int):
        ...
        num_edge_types = 2
        cur_x = torch.cat([x for _ in range(num_edge_types)], dim=0)  # 4V, D
        loop_edge_list = self.matrix_loop_new(edge_index_list, max_node_per_graph)  # 4V, 4V
        out = self.conv1(cur_x, loop_edge_list)  # 4V, D
        out = out.view(self.num_edge_types, x.shape[0], out.shape[-1])  # 4, V, D
        ...
        return self.aggr(out, graph_idx, batch_size)

    def matrix_loop_new(self, edge_list, max_node_per_graph):
        n = max_node_per_graph
