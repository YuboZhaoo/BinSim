...
public class ModelCounter {
  ...
  public ModelCounter(int bound, String mode) {
    ...
    this.abc.setOption(Option.DISABLE_EQUIVALENCE_CLASSES);
    this.bound = bound;
...
//      }
      ...
      if (model_counting_vars.size() > 1) {
        for(Pair<String,String> var : model_counting_vars) {
          if(var.second.equals("java.lang.String"))
            numStringVar++;
          else
            numIntVar++;
        }
      ...
      if(model_counting_vars.size() > 1 && numStringVar == 0) {
        ...
        count = new BigDecimal(1);
        if(multiTrackissueFlag) {
          count = new BigDecimal(abc.countVariable(model_counting_vars.get(0).first, bound));
        } else {
          for (Pair<String,String> var : model_counting_vars) {
            if(var.first.equals("java.lang.Character"))
              continue;
            count = count.multiply(new BigDecimal(abc.countVariable(var.first, bound)));
          }
        }
      }
